...
def test_render_html_param_filename_takes_string():
    assert_raises(TypeError, views.render_html, None, None,
                  exc_pattern=r'macherie.views.render_html ' \
                  'takes a string as filename param, got None.')
    assert_raises(TypeError, views.render_html, 5, None,
                  exc_pattern=r'macherie.views.render_html ' \
                  'takes a string as filename param, got 5.')

def test_render_html_param_filename_should_not_be_empty():
    assert_raises(TypeError, views.render_html, '', None,
                  exc_pattern=r'macherie.views.render_html ' \
                  'filename param can not be empty.')

def test_render_html_param_context_takes_dict():
    assert_raises(TypeError, views.render_html, 'index.html', None,
                  exc_pattern=r'macherie.views.render_html ' \
...
def test_render_html_complains_cherrypy_not_configured_when_no_template_path_specified():
    assert_raises(LookupError, views.render_html, 'index.html', {},
                  exc_pattern=r'You must configure "views.dir" ' \
                  'string in CherryPy or pass template_path param ' \
                  'to render_html')

def test_render_html_param_template_path_takes_string():
    assert_raises(TypeError, views.render_html, 'index.html', {},
                  template_path=1,
                  exc_pattern=r'macherie.views.render_html ' \
...
def test_render_html():
    got = views.render_html('test1.html', dict(title='foo', header='bar'), template_path=templates)
